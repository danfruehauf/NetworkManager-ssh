...
send_network_config (NMSshPlugin *plugin)
{
	...
	else
	{
		...
		{
			device = (gpointer) g_strdup_printf ("%s%d", io_data->dev_type, io_data->local_dev_number);
	...
	GError **error)
{
	...
	if (!strncmp (auth_type, NM_SSH_AUTH_TYPE_PASSWORD, strlen(NM_SSH_AUTH_TYPE_PASSWORD))) {
		...
		if (!strncmp (auth_type, NM_SSH_AUTH_TYPE_KEY, strlen(NM_SSH_AUTH_TYPE_KEY))) {
			...
			if (ssh_private_key_tmp && strlen (ssh_private_key_tmp)) {
				...
				g_free (ssh_private_key_tmp);
				ssh_private_key_tmp = NULL;
			} else {
	...
	if (tmp && strlen (tmp)) {
		priv->io_data->socks_bind_address = g_strdup (tmp);
	} else {
		priv->io_data->socks_bind_address = NULL;
	}
	...
	if (priv->io_data->no_tunnel || priv->io_data->no_remote_command) {
		...
		{
			tmp_arg = g_strconcat(ip_addr_cmd_4, "; ", ip_link_cmd, NULL);
		}
...
main (int argc, char *argv[])
{
	...
	if (system ("/sbin/modprobe dummy numdummies=8") == -1)
		exit (EXIT_FAILURE);
